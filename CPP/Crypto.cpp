...
std::vector<int> Crypto::extract_LSBits(SecretImage& secret_image, int message_length) {
    ...
    if (total_bits_needed > total_pixels) {
        throw std::runtime_error("Insufficient plixes.");
    }
    ...
    int start_index = total_pixels - total_bits_needed;

    // 6. Extract LSBs from the image pixels and return the result.
     for (int i = start_index; i < total_pixels; ++i) {
        int row = i / width;
        int col = i % width;
...
std::string Crypto::decrypt_message(const std::vector<int>& LSB_array) {
    ...
    if (LSB_array.size() % 7 != 0) {
        throw std::runtime_error("Wrong LSB array size.");
    }
    ...
    for (size_t i = 0; i < LSB_array.size(); i += 7) {
        ...
        // Building ASCII
        for (int j = 0; j < 7; ++j) {
            charCode = (charCode << 1) | LSB_array[i + j];
        }

        // 3. Collect the characters to form the decrypted message.
        message += static_cast<char>(charCode); // Static cast char turns 7 bits to ASCII
    }
...
std::vector<int> Crypto::encrypt_message(const std::string& message) {
    ...
    //    You can use std::bitset.
        ...
        for (int i = 6; i >= 0; --i) { // Doing it in reverse so that the MSB is at start
...
SecretImage Crypto::embed_LSBits(GrayscaleImage& image, const std::vector<int>& LSB_array) {
    ...
    int start_pixel = total_pixels - LSB_array.size();  // Starting from first secret pixel

    // 3. Iterate over the image pixels, embedding LSBs from the array.
    int lsb_index = 0;  // Iterator cariable
    for (int i = start_pixel; i < total_pixels && lsb_index < LSB_array.size(); ++i) {
        int x = i / w;
        int y = i % w;

        int pixel_value = image.get_pixel(x, y);
        pixel_value = (pixel_value & ~1) | LSB_array[lsb_index];  // Change LSB
