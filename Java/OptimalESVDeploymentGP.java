...
public class OptimalESVDeploymentGP
{
    ...
    public int getMinNumESVsToDeploy(int maxNumberOfAvailableESVs, int maxESVCapacity)
    {
        ...
        e1: for(Integer energy: maintenanceTaskEnergyDemands){
            for(ArrayList<Integer> ESV: maintenanceTasksAssignedToESVs){
                int remain = 0;
                for(Integer used: ESV){
                    remain += used;
                }
                if (remain + energy <= maxESVCapacity){
                    ESV.add(energy);
                    continue e1;
                }
            }
            ArrayList<Integer> tempESV = new ArrayList<>();
            tempESV.add(energy);
            maintenanceTasksAssignedToESVs.add(tempESV);
        }
